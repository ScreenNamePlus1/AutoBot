...
def yagi_calculator():
    ...
    # A 3-element Yagi (Reflector, Driven, 1 Director) typically has ~7-8 dBi gain
    # Each additional director adds a decreasing amount of gain
    if num_directors >= 1:
        # Simple logarithmic model for gain
        gain_dBi = 7.0 + (num_directors * 1.5)
    else:
        # A 2-element Yagi (Reflector, Driven) has less gain
        gain_dBi = 5.0
    ...
    for i in range(1, num_directors + 1):
        # Directors get progressively shorter as you go further from the driven element
        # This is a common design method
        director_m = (0.440 * wavelength_m) - end_effect_factor
        director_length_adj = (i * 0.005) * wavelength_m # small reduction for each director
